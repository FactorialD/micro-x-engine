...
public final class Telemetry {
    ...
    private long epoch;
    private final short[] updates = new short[64], renders = new short[64], scratch = new short[64];
    public void timing(int update, int render) {
        updateMs = update;
        renderMs = render;
        int p = samples & 63;
        updates[p] = (short) Math.min(32767, update);
        renders[p] = (short) Math.min(32767, render);
        samples++;
    }
    public void rendererBudget(int used, int budget) {
    ...
    public void frame(long now) {
        ...
        if (now - epoch >= 1000) {
            ...
            if (used > peakUsedMemory)
                peakUsedMemory = used;
            int n = Math.min(samples, 64);
            updateP50 = percentile(updates, n, 50);
            updateP95 = percentile(updates, n, 95);
    ...
    private int percentile(short[] source, int n, int percent) {
        if (n == 0)
            return 0;
        for (int i = 0; i < n; i++) scratch[i] = source[i];
        for (int i = 1; i < n; i++) {
            short v = scratch[i];
            int j = i - 1;
        ...
        return scratch[((n - 1) * percent) / 100];
    }
}
