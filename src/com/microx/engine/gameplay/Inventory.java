...
public final class Inventory {
    private final short[] ids, counts, durability;
    private final int cellLimit;
    private int money;
    public Inventory(int slots, int cells) {
        if (slots < 1 || cells < 1)
            throw new IllegalArgumentException();
        ids = new short[slots];
        counts = new short[slots];
        durability = new short[slots];
        cellLimit = cells;
    }
    public int slots() {
        return ids.length;
    ...
    public int countAt(int i) {
        check(i);
        return counts[i] & 65535;
    }
    public int durabilityAt(int i) {
        check(i);
        return durability[i] & 65535;
    }
    public int money() {
        return money;
    }
    ...
    public void clear() {
        for (int i = 0; i < ids.length; i++) ids[i] = counts[i] = durability[i] = 0;
        money = 0;
    }
    ...
    public boolean canAdd(int id, int amount) {
        if (!ItemCatalog.valid(id) || amount <= 0)
            return false;
        int left = amount;
        for (int i = 0; i < ids.length; i++)
            if (ids[i] == id)
    ...
    public boolean canAddAfterRemoving(int addId, int amount, int removeId) {
        if (count(removeId) < 1)
            return false;
        int slot = -1;
        ...
        short oldId = ids[slot], oldCount = counts[slot], oldDurability = durability[slot];
        counts[slot]--;
        if (counts[slot] == 0) {
            ids[slot] = 0;
            durability[slot] = 0;
        }
        boolean result = canAdd(addId, amount);
        ids[slot] = oldId;
        counts[slot] = oldCount;
        durability[slot] = oldDurability;
        return result;
    }
    ...
    public boolean remove(int id, int amount) {
        if (amount <= 0 || count(id) < amount)
            return false;
        int left = amount;
        for (int i = ids.length - 1; i >= 0 && left > 0; i--)
            if (ids[i] == id) {
                int n = Math.min(left, counts[i] & 65535);
                counts[i] -= n;
                left -= n;
                if (counts[i] == 0) {
                    ids[i] = 0;
    ...
    public int conditionOf(int id) {
        for (int i = 0; i < ids.length; i++)
            if (ids[i] == id)
                return durability[i] & 65535;
        return 100;
    }
    public boolean setCondition(int id, int value) {
        if (value < 0 || value > 100)
            return false;
        for (int i = 0; i < ids.length; i++)
            if (ids[i] == id) {
                durability[i] = (short) value;
                return true;
            }
        return false;
    }
