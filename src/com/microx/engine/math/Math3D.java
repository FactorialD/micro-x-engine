    ...
    public static void multiply(int[] a, int[] b, int[] out) {
        ...
        for (row = 0; row < 4; row++)
            for (column = 0; column < 4; column++) {
                long sum = 0;
                for (k = 0; k < 4; k++)
                    sum += ((long) a[row * 4 + k] * b[k * 4 + column]) / Fixed.ONE;
    ...
    public static int clipTriangleNear(int[] triangle, int offset, int near, int[] output) {
        ...
        for (i = 0; i < 3; i++) {
            ...
            if (cin != pin) {
                ...
                output[count * 3 + 1] =
                        Fixed.lerp(triangle[previous + 1], triangle[current + 1], t);
                output[count * 3 + 2] = near;
                count++;
            }
            if (cin) {
                output[count * 3] = triangle[current];
                output[count * 3 + 1] = triangle[current + 1];
                output[count * 3 + 2] = triangle[current + 2];
                count++;
            }
        }
        return count;
